...
- **父进程退出**：某些系统不允许子进程在父进程终止后继续运行（如“孤儿进程”会被清理）。
...
+ **Long-term scheduler 或 job scheduler：**选择哪些进程被**加载到内存中**(ready queue) 或者说被执行，调用频率**较低**sec min，控制**可同时运行的最大进程数**(degree of multiple programming )
+ **Medium-term Scheduler：**从内存中**移除**一个进程并将其储存到磁盘上(挂起)，当需要时再从磁盘中**重新加载**到内存中执行(恢复)，**swap-in**从磁盘中加载到内存，**swap-out**从内存加载到磁盘。可以提高内存利用率，实现了虚拟内存，基于优先级调度，但是性能下降，数据容易丢失，page fault增加
