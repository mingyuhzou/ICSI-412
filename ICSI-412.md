...
**fork()会返回两次，子进程会获得pid=0，父进程会获得子进程的标识符**，如果失败(一般是达到可以创建的**上限**)会返回**-1**
...
文件是**比特的序列**，内核并没有使用**数据结构**来组织文件，文件按照**树形结构的目录**组织(Tree-Structured Directories)，目录本身也是一个**文件**，储存了**如何寻找其他文件的内容**。
