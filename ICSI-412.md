...
线程是CPU**资源分配和执行**的**最小**单位(CPU utilization)，可以看作是**按顺序**(sequential)执行的一系列指令。
...
通过流水线技术可以**提高吞吐量(1/最长耗时阶段)**，但并**不能降低延时**反而提高，每个阶段**固定**为最长耗时阶段
...
Socket是一个**抽象层**，通过它，应用程序可以**发送和接收数据**。它类似于打开一个文件句柄(open-file handle)，应用程序通过文件句柄与文件进行读写操作，而应用程序通过socket与网络中的**其他应用**进行数据交换。
...
主机之间的通信通过**两个IP地址和两个端口号**来标识，同时**底层协议**(TCP/UDP)也会影响通信的区分。
...
Cache（高速缓存）：是**容量很小**但是**非常快、也很贵**的内存，主要用来存放最近用到的数据，减少访问主内存的时间。
...
固定分区Fixed partitions，分区的大小在**启动时就已确定**，分区的大小**可以相同也可以不同**，但长度**不会变化**。
...
如果分区的大小不同，那么每个进程会分配给**刚好能装下它的最小分区**。

每个分区有自己的**队列**，当分区空闲时进程可以**直接装入**，否则新来的进程要在对应的队列中**排队等待**。
...
如果进程大多是**I/O密集型**的，那么需要放**更多的进程**来弥补CPU空闲，**进程越多CPU利用率越高**
...
+ **最佳适应算法 Best-fit：选择最小的足够大的空闲内存块，总体性能最差，它要找所有的分区，并且会留下许多小的不连续的空间因此需要经常压缩。**
...
+ **重定位relocation**：操作系统不能确定程序被加载到内存的那个位置，因此变量和程序不能使用**绝对内存地址**，需要支持重定位才能将程序的代码和数据迁移到不同的位置而不改变程序的逻辑和行为
+ **保护**：防止进程访问**其他**进程的内存，防止进程**修改自身的内存敏感区域**(代码段)



**base register** 记录进程的开始位置（物理地址），**bound register**记录进程的结束位置，**limit register** 最大地址偏移量。

它们的值在进程载入load或swap in 时设置，这两个寄存器只有在**系统模式下**(system mode)才能访问。

- **虚拟地址Logical Address，进程视角下的地址**

- **物理地址Physical address，实际内存中的地址，Physical address = base + logical address**

当程序执行时，所有的相对地址都会加上基址寄存器的地址得到一个绝对地址(物理地址)，然后与界限寄存器中的值**比较**，确保该地址在进程的内存范围内，实际上直接比较limit register的值与logical 地址即可，因为虚拟内存是从**0**开始的。

<img src="./assets/image-20250427155233878.png" alt="image-20250427155233878" style="zoom: 50%;" />



**bitmap**可以记录内存使用，每一位对应**固定大小**的**一块内存区域**，**块越小需要的内存就越多**，**1**表示已分配，**0**表示空闲。位图的大小只和总内存有关与**分配状态无关**，如果1bit表示4KB，那么1MB需要256bit。

寻找空闲区域需要扫描位图，**不容易快速找到**，但是修改状态只需**翻转**。
...
**链表**也可以记录内存使用，每一个条目表示空闲或已使用的连续内存区域，节点记录两个信息，**起始位置—内存大小**

![image-20250427162041698](./assets/image-20250427162041698.png)



当内存被划分为**较大的连续块**时，链表的管理效率较高，因为**节点的数量较少**。

![image-20250427162618255](./assets/image-20250427162618255.png)



交换技术**swapping**是一种常见的内存管理技术，用于**将不活跃的进程移出内存**，腾出空间给其他进程。这种技术会带来一些问题

+ 交换技术要求**整个**进程必须能够进入物理内存，因此**无法运行比物理内存还大的进程**(虚拟内存可以实现)
+ 内存碎片化**fragmented**
+ 进程**要么完全在内存中，要么完全在磁盘中**，即使只交换**一小部分**内存也必须**交换整个进程**



覆盖技术**overlays**，**分批加载进程的不同部分到内存中(一般是数据部分)**，只将当前**需要**的代码/数据留在内存中，动态替换非必须的部分，用户自行实现**不依赖操作系统的特殊支持**，但编程设计较为**复杂**。

<img src="./assets/image-20250427164929677.png" alt="image-20250427164929677" style="zoom:50%;" />

overlays可以解决**第一个问题**，仍然**无法解决碎片化fragmented**和**进程部分驻留**partially resident processes(仍要频繁地交换)



#  Virtual Memory

进程使用**虚拟地址空间**，每个进程他都有自己**独立**的地址空间，其可以**大于**物理内存，在任意时刻只有**一部分**虚拟空间地址映射到物理内存，**部分进程**的内存内容会被**保存在磁盘上**，硬件和操作系统合作，将内容在磁盘和物理内存之间移动。



**Demand Paging**按需分页：**只有在需要的时候才会将页调入内存，减少I/O调用，内存使用，响应速度更快，提高并发的数量。**



为了知道某个页面**是否在内存中**，操作系统需要硬件的帮助

+ 每个页表条目page table entry包含一个**有效-无效位valid-invalid bit**，有效则表示页加载到了内存中，无效则是当前页还在磁盘上，访问时会发生**page fault**操作系统将其从磁盘调入内存。



![image-20250430084411872](./assets/image-20250430084411872.png)
...
+ **读取页面**
+ 完成后，**磁盘发出一个中断**
+ **保存当前运行进程的上下文(CPU处理别的进程)，更新页表，将页面标记为valid，将该进程放入就绪队列**
+ **再次执行导致缺页的指令**



<img src="./assets/image-20250430084745776.png" alt="image-20250430084745776" style="zoom:50%;" />

**Page Fault Rate**：表示访问内存时发生Page Fault的概率。

有Page Fault时的平均访问时间计算公式：
$$
\text{Avg. access time} = (1 - p) \times \text{memory access time} + p \times \text{page fault time}
$$
对效率影响很大，需要降低**Page Fault Rate**



**Page Replacement（页面置换）**，**只有实际需要的时候才加载页面**，没有空闲页框时(内存已满)，系统需要选一个已有的页框将其**腾空**。



**Optimal Page Replacement** 最佳页面置换，置换时选择未来最长时间不会用到的页面，**page fault rate 最低**，不过**不可能实现**。

![image-20250430090038941](./assets/image-20250430090038941.png)



**FIFO Page Replacement Algorithm**（先进先出页面置换算法），将**最早**加载到内存的页面淘汰掉，**逻辑直观易于实现**，性能不稳定，**可能会替换掉经常使用的**。

![image-20250430090348988](./assets/image-20250430090348988.png)





**LRU（Least Recently Used）**置换**最久**未使用的页，但是每个页表项有**一个时间字段**，同时CPU加上了一个**计数器**，每次访问页面都要**更新时间并搜索页表增加开销**。使用栈维护页号，当页面被使用时，会将页号从栈中移除(用**双向链表**实现)再放到**栈顶**，使得最不常用的在栈底

![image-20250430090638295](./assets/image-20250430090638295.png)

可以通过硬件设置**引用位Reference bit**优化，**初始时都为0**，当页面被访问设置为**1**，淘汰时遍历**所有**页面的引用位，**1置为0**，**丢弃第一个0**，不过仍然**很慢**。

每个页表项都配备了一个8位的引用位 0000000(**最高位表示最近时间段**)，**数值越大表示最近被使用过**，11000100比01110111晚被使用，**每隔一段时间会发生一次中断**，将所有页的引用位**右移**一位，如果这个时间内使用了该页那么**插入1**，**反之0**。

LRU契合**局部性原理**，Temporal locality和Spatial locality



**2nd Chance Page Replacement**，改进的FIFO，页表项添加一个**标志位R**——**是否最近被访问过**，**新页面载入时设置为1**，置换时检查**栈底**如果**R=1则置为0**并放到栈顶**继续检查下一个**，反之置换掉



working set，进程在特定时间段内集中**访问的页面集合**
...
如果某段代码是**只读的**(reentrant)，那么不同进程之间可以**共享**这一份代码的副本，必须加载到所有进程的**相同虚拟地址**(不是页表)
...
文件系统指的是**整个目录树(整个目录层次)**，用于组织电脑上的文件，通常储存在**磁盘**上，磁盘可以划分为**多个分区**partition，每个分区都有**独立**的文件系统

**扇区sector**是磁盘的**最小单位**，磁盘的**0级**sector是**MBR**(Master Boot Record)，分区表**partition table**位于MBR**尾部**包含**每个分区的起始和结束位置，其中有一个分区被标记为active活跃分区。**
...
+ I-nodes（索引节点）：每个文件对应一个**固定大小**的**i-node数据结构**，其中**存放文件属性以及指向文件数据块的指针(直接/间接)**，操作系统可以通过i-node找到文件**所有的数据块**。当直接数据块指针用完了会将使用**预留**的**最后一个指针**(间接指针)，该指针指向一个**专门存储额外块的列表**的磁盘块
  + **减少内存使用**
  + **更新复杂度较高，多级指针**
...
文件用数据块表示，那么**目录也是用数据块表示的**，目录的数据块中**会额外维护目录项数组**，因此可以继续找。
...
2. 磁盘收到命令后，定位到正确的扇区将数据通过**DMA**以直接内存访问的方式传输到内存中，**无需CPU参与**
...
DMA在传输数据时会**绕过CPU**，**直接在内存和设备之间传输**。操作系统会将**DMA命令写到内存中**，其中包括了**数据的源/目标地址，以及多少字节，读还是写**，随后会将这个命令的**地址**告诉**DMA控制器**，让其开始工作。传输数据时，DMA控制器会**暂时夺取总线bus**的控制权。注意这里的命令是告诉DMA怎么**把数据在磁盘和内存间转移**，而不是如何去磁盘中读数据。
...
- 如果发生**进位**（即加法的结果超出了数据的位宽），则将进位加回到结果中，这种加法被称为 **环绕进位**（end-around carry）。
