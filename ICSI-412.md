...
通过fork可以创建一个和父进程有**相同状态**的子进程(子进程也会有自己**不同的资源**，比如pid)，但该进程拥有自己**独立的空间**；子进程继承父进程的**文件描述符(表)**(注意不是拥有**同一个**)，即共同操作文件，对同一文件的写入**不会被覆盖但是顺序不确定**；子进程从**fork的下一行**开始执行，如果父进程**重定向了输出**，那么fork出的子进程的输出也同样会被重定向。
...
CPU-I/O周期：进程的执行通常不是**连续的CPU计算**，而是交替进行**计算(CPU)和I/O操作(由I/O设备处理，不需要CPU)**，形成一个周期。

+ **CPU Burst（CPU 执行阶段）**一般跟在I/O Burst后面
+ **I/O Burst（I/O 等待阶段）**
...
+ **Short-term Scheduler 或 CPU Scheduler：**选择CPU**下一个**要执行的进程，执行频率**较快**milliseconds，可能是系统中的**唯一**的调度器，当**事件**触发时(Clock interrupts I/O interrupts Operating system calls Signals)会被调用。
+ **Long-term scheduler 或 job scheduler：**选择哪些进程被**加载到内存中**(ready queue) 或者说被执行，调用频率**较低**sec min
+ **Medium-term Scheduler：**从内存中**移除**一个进程并将其储存到磁盘上(挂起)，当需要时再从磁盘中**重新加载**到内存中执行(恢复)，因为内存是有限的，当进程太多时会通过**swapping**机制处理，**swap-in**从磁盘中加载到内存(取决于**Degree of Multiprogramming**)，**swap-out**从内存加载到磁盘。可以提高内存利用率，实现了虚拟内存，基于优先级调度，但是性能下降，数据容易丢失，page fault增加

+ **I/O scheduling：**决定哪个进程的**I/O请求**被I/O设备处理。

<img src="./assets/image-20250321143245368.png" alt="image-20250321143245368" style="zoom: 67%;" />
...
+ 作业队列 **job queue** 储存**系统中所有的进程，还未进入内存**
+ 就绪队列**ready queue** **存储放入内存，并且等待CPU执行的进程**
+ 储存设备队列 **device queue**  **等待I/O设备完成操作的进程**。进程在这三种队列中**转换**。
+ **Suspended Queue**：挂起的进程队列。
+ **Waiting Queue**：等待某种事件的进程（比如信号量/互斥量）队列。
...
Long-term scheduler负责**平衡 ** I/O-bound进程和CPU-bound进程的**比例**，**防止系统在运行时因为I/O等待过多而陷入空闲，或者CPU资源被计算密集型进程沾满而导致其他系统饥荒。**
...
+ 抢占式：每个进程会运行**固定的时间**(在时间间隔结束时发生clock中断)或者**其提前结束**
...
因此如果存在**长作业位于队首**时会导致**平均等待时间**较大，比如CPU密集型后跟多个I/O密集型。
...
CPU burst 是未知的，一般通过历史数据预测，a等于0.8——权重集中在**最近 4 次观测值上**，能够**快速反应数据变化**，如果**有异常值**，变化会出现波动。当 α = 0.2权重分布在**最近 8 次观测值上**，意味着**过去**的数据仍然对当前平均值有显著影响。

<img src="./assets/image-20250321153602041.png" alt="image-20250321153602041" style="zoom:67%;" />
...
在优先级调度中，**最小**的数字表示**最大**的权重。**SJF也是一种优先级调度**，其中优先级就是运行时间。显然地，优先级调度会出现starvation的问题，解决方法是**Aging**即通过时间来增加进程的优先级。
...
轮转调度，每个进程都会被分配一个**固定的时间片(Time Quantum)**，如果进程未在q时间内完成则会触发时钟中断，进程会被暂停，并放入**就绪队列的末尾**。
...
多级反馈队列调度，进程可以在队列之间**切换**——如果一个进程用了**太长的CPU时间**，那么它会被移动到**低优先级队列**中；**I/O密集型进程和交互式进程**通常会被保留在**较高优先级**的队列中；如果一个进程在**较低优先级**的队列中等待了太长时间，它可能会被移动到**较高优先级的队列**中。

<img src="./assets/image-20250323215405463.png" alt="image-20250323215405463" style="zoom:50%;" />

调度必须在**队列间进行**，调度策略有两种：1.队列之间有**固定的优先级**。2. **划分时间片**，每个队列会的一定的CPU时间用于调度内部进程。

一个多级反馈的例子是

<img src="./assets/image-20250326170447074.png" alt="image-20250326170447074" style="zoom:50%;" />

1. 进入到执行队列进程会被放入到**最高优先级**的队列
2. 如果进程在**规定的时间片**内完成，则直接**退出**，反之进入到**下一级**队列中并获得**更长的时间片**
...
当向**已满**的管道写入数据时，系统会**阻塞**程序，直到管道有**足够的空间**。如果从**空的**管道中读取也同样会**阻塞**。

同时，如果一个进程**打开管道用于读取**，而没有进程**打开管道写入**，读取进程会被一直**阻塞**，即管道需要**匹配**。



当写端关闭，读端会读取到0，表示EOF。反之，write()会返回**SIGPIPE**信号，如果忽略信号则 `write()` 返回 `-1`，`errno` 设为 `EPIPE`
...
dup2(oldfd,newfd)相较于dup，**可以指定将oldfd复制给谁**
